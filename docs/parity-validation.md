...
|---|---|---|
| Strict nested structured JSON output | `OutputSchema`, `tests/test_output_schema.py`, `tests/test_schema_interchange.py`, `tests/test_one_of_schema.py`, `tests/test_schema_all_of.py`, positional-array and local-definition tests | Partial: JSON types, homogeneous/positional arrays, typed suffixes, typed maps, properties, required/extra keys, scalar enums, numeric/length bounds, bounded `anyOf`/`oneOf`/`allOf`/nullability and root-local acyclic `$defs`/`$ref`. Recursive/external references, keyword-only and general Boolean subschemas, discriminators and full schema dialect test corpus remain open. |
